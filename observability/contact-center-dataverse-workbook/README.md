...
The lifecycle KQL matches the supplied ID before classification and requires both a conversation resource ID and subscenario. It maps canonical events such as `ConversationCreated`, `RouteToQueue`, `AgentAssignment`, `CSRAssignment`, `CSRAccepted`, `CustomerEndedConversation`, and `CSREndedConversation` into business stages. Any conversation subscenario that does not have a specialized mapping is safely summarized under `Conversation`, so new or workload-specific event names do not make the graph empty. Nodes show a plain-language outcome and stage timing. Connectors show elapsed time between stages. Assignment failures and timeouts are surfaced as `Needs attention`, but raw subscenario, action, event, operation, agent, and call fields remain on the Investigator page.
