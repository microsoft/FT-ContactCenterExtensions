...
The lifecycle KQL normalizes the conversation identity from `powerplatform.analytics.omnichannel.conversation.id`, `omnichannel.conversation.id`, or `powerplatform.analytics.resource.id`. This keeps call-level events attached to their parent conversation even when their resource ID is the call ID. It also accepts `powerplatform.analytics.correlation.id` in the investigation filter. Canonical events including `ConversationCreated`, `RouteToQueue`, `CSRAssignment`, `CSRAccepted`, `CallConnected`, `WaitForAgent`, `CallEnded`, `CustomerEndedConversation`, and `CSREndedConversation` map into business stages. Any conversation subscenario without a specialized mapping is safely summarized under `Conversation`, so new or workload-specific event names do not make the graph empty. The graph uses the full page width and full control height, with nodes rendered at twice the original size and stage names shown as the dominant node text. Nodes also show a plain-language outcome and milestone time. Connectors show elapsed time between first occurrences of stages, preventing repeated agent events from producing negative durations. Assignment failures and timeouts are surfaced as `Needs attention`, but raw telemetry remains on the Investigator page.
