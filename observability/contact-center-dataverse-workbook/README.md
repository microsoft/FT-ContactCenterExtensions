...
The lifecycle KQL first requires `powerplatform.analytics.scenario == "ConversationDiagnosticsScenario"` so queue-capacity and other operational signals cannot be mistaken for a customer journey. It maps canonical events such as `ConversationCreated`, `RouteToQueue`, `AgentAssignment`, `CSRAssignment`, `CSRAccepted`, `CustomerEndedConversation`, and `CSREndedConversation` into business stages, summarizes each stage, and joins only the stages observed for the selected conversation. Nodes show a plain-language outcome and stage timing. Connectors show elapsed time between stages. Assignment failures and timeouts are surfaced as `Needs attention`, but raw subscenario, action, event, operation, agent, and call fields remain on the Investigator page.
